...
Move_Log_Element* Create_Element_Move_Log(){
	...
	elem->en_passant_type_done = NO_EN_PASSANT; // initializes en_passant_type_done to NO_EN_PASSANT (since no en_passant has been made yet)
	elem->color_of_pawn_promoted = NO_COLOR; // initializes color_of_pawn_promoted to NO_COLOR (since no pawn has been promoted yet)
	elem->type_of_pawn_promoted = NOTHING; // initializes type_of_pawn_promoted to NOTHING (since no pawn has been promoted yet)
	elem->pawn_move_made = false; // initializes pawn_move_made to false (since no pawn has been moved yet)
...
Move_Log_Element* Copy_Move_Log_Element(Move_Log_Element* elem){
	...
	copy->en_passant_type_done = elem->en_passant_type_done;
	copy->color_of_pawn_promoted = elem->color_of_pawn_promoted;
	copy->type_of_pawn_promoted = elem->type_of_pawn_promoted;
	copy->pawn_move_made = elem->pawn_move_made;
...
void Change_Move_Log_Element(Move_Log_Element* elem, int previous_row, int previous_col, int destination_row, int destination_col, int check_state, int taken_piece_type, int taken_piece_color, int rock_type, int en_passant_type, int color_of_pawn_promoted, int type_of_pawn_promoted, int pawn_move_made){
	...
	elem->en_passant_type_done = en_passant_type;
	elem->color_of_pawn_promoted = color_of_pawn_promoted;
	elem->type_of_pawn_promoted = type_of_pawn_promoted;
	elem->pawn_move_made = pawn_move_made;
}
...
Move_Log_array_MESSAGE_TYPE Add_Element_in_Move_Log_array_at_Index(Move_Log_array* Moves_Log, Move_Log_Element* elem, int index){
	...
	if (index < Moves_Log->actual_size){// if index is less then actual_size
		for (int i=Moves_Log->actual_size-1; i>=index; i--){ // shifts all other Move_Log accordingly by one position to the right, the loop is in this order to avoid not overwriting the previous element
			...
			Moves_Log->Move_Log[i+1]->en_passant_type_done = Moves_Log->Move_Log[i]->en_passant_type_done;
			Moves_Log->Move_Log[i+1]->color_of_pawn_promoted = Moves_Log->Move_Log[i]->color_of_pawn_promoted;
			Moves_Log->Move_Log[i+1]->type_of_pawn_promoted = Moves_Log->Move_Log[i]->type_of_pawn_promoted;
			Moves_Log->Move_Log[i+1]->pawn_move_made = Moves_Log->Move_Log[i]->pawn_move_made;
		...
		Change_Move_Log_Element(Moves_Log->Move_Log[index], elem->move->previous_row, elem->move->previous_col, elem->move->destination_row, elem->move->destination_col, elem->check_state, elem->taken_piece_type, elem->taken_piece_color, elem->rock_type, elem->en_passant_type_done, elem->color_of_pawn_promoted, elem->type_of_pawn_promoted, elem->pawn_move_made);
	}
