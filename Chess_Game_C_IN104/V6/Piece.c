...
bool Is_Pawn_Promotion_Possible(Move* move, Piece*** board){
    ...
    if (board[move->previous_row][move->previous_col]->color == WHITE && board[move->previous_row][move->previous_col]->type == PAWN && move->previous_row == 1){
        // if nothing is in the straigh line of the pawn
        if (board[move->previous_row-1][move->previous_col]->type == NOTHING && move->previous_col == move->destination_col && move->previous_row-1 == move->destination_row){
            return true;
        }
        // looking for a piece of the opposite color in the left diagonal
        if (move->previous_col-1>=0 && move->previous_col-1 == move->destination_col && move->previous_row-1 == move->destination_row&& board[move->previous_row-1][move->previous_col-1]->type != NOTHING && board[move->previous_row-1][move->previous_col-1]->color != board[move->previous_row][move->previous_col]->color){
            return true;
        }
        // looking for a piece of the opposite color in the right diagonal
        if (move->previous_col+1<8 && move->previous_col+1 == move->destination_col && move->previous_row-1 == move->destination_row && board[move->previous_row-1][move->previous_col+1]->type != NOTHING && board[move->previous_row-1][move->previous_col+1]->color != board[move->previous_row][move->previous_col]->color){
            return true;
        }
    }

    // if the pawn is black and it's on the 8st row
    if (board[move->previous_row][move->previous_col]->color == BLACK && board[move->previous_row][move->previous_col]->type == PAWN && move->previous_row == 6){
        // if nothing is in the straigh line of the pawn
        if (board[move->previous_row+1][move->previous_col]->type == NOTHING && move->previous_col == move->destination_col && move->previous_row+1 == move->destination_row){
            return true;
        }
...
int Is_Rock_Possible(Move* move, State_Of_Rock_and_Check* State_Of_Rock_and_Check, Piece*** board){
    ...
    && State_Of_Rock_and_Check->is_white_king_checked == false && State_Of_Rock_and_Check->white_king_moved == false && State_Of_Rock_and_Check->white_rock_done == false){
        ...
        if (move->previous_col == 4 && move->destination_col == 0 && State_Of_Rock_and_Check->white_left_rook_moved == false && board[7][0]->type == ROOK && board[7][0]->color == WHITE && board[move->destination_row][move->destination_col]->color == WHITE){
        ...
        else if (move->destination_col == 7 && move->previous_col == 4 && State_Of_Rock_and_Check->white_right_rook_moved == false && board[7][7]->type == ROOK && board[7][7]->color == WHITE && board[move->destination_row][move->destination_col]->color == WHITE){
    ...
    && State_Of_Rock_and_Check->is_black_king_checked == false && State_Of_Rock_and_Check->black_king_moved == false && State_Of_Rock_and_Check->black_rock_done == false){
        ...
        if (move->previous_col == 4 && move->destination_col == 0 && State_Of_Rock_and_Check->black_left_rook_moved == false && board[0][0]->type == ROOK && board[0][0]->color == BLACK && board[move->destination_row][move->destination_col]->color == BLACK){
            // the case bewteen the king and the rook needs to be empty of pieces
            for (int j = 1; j < 4; j++){
        ...
        // if the king is moving to the right, the right rook needs to be on its start position and not moved
        else if (move->destination_col == 7 && move->previous_col == 4 && State_Of_Rock_and_Check->black_right_rook_moved == false && board[0][7]->type == ROOK && board[0][7]->color == BLACK && board[move->destination_row][move->destination_col]->color == BLACK){
            // the case bewteen the king and the rook needs to be empty of pieces
            for (int j = 5; j < 7; j++){
                if (board[0][j]->type != NOTHING || Is_Case_threatened(BLACK, 0, j, board) == true){
                    return NO_ROCK;
