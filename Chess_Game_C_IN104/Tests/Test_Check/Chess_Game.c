...
int main (){
    ...
    while (is_running_game != -1) {
                ...
                if (row >= 0 && row < BOARD_SIZE && col >= 0 && col < BOARD_SIZE && has_match_started == true && is_player_playing_human == true && is_pawn_promotion_happening == false){
                    ...
                    if (is_clicked_1 == 1 && is_clicked_2 == 1) {
                        ...
                        if (Is_Move_Valid(move, board, State_Of_RockandCheck, Pawn_Move_State) == true){
                            ...
                            /*
                            bool is_check = Is_Check(color_we_are_checking, board);
                            if (is_check == true && board[Log->Move_Log[Log->actual_size-1]->move->destination_row][Log->Move_Log[Log->actual_size-1]->move->destination_col]->type == KING){
                            ...
                            */
                            ...
                            /*
                            // to see if the case threatened is working as the check
                            bool is_case_threathen = Is_Case_threatened(WHITE, 7, 4, board);
                            if (is_case_threathen == true){
                                printf("The case of the white king is threatened (not with the check)\n");
                            }
                            else {
                                printf("The case of the white king is not threatened (not with the check)\n");
                            }
                            */
        ...
        if (check_mate_state == WHITE_CHECKMATE || check_mate_state == BLACK_CHECKMATE){
        ...
        if (loosing_player != -1){
            is_running_game = -1;
            printf("The player that has lost is : %d\n", loosing_player);
        }
